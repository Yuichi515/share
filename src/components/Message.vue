...
export default {
  ...
  methods: {
      ...
      if (result) {
        this.shares[index].like.forEach((element) => {
          if (element.user_id == this.$store.state.user.id) {
            axios({
              method: "delete",
              url: "https://infinite-atoll-83522.herokuapp.com/api/like",
              data: {
        ...
        axios
          .post("https://infinite-atoll-83522.herokuapp.com/api/like", {
            share_id: this.shares[index].item.id,
            user_id: this.$store.state.user.id,
          })
          .then((response) => {
    ...
    del(index) {
      axios
        .delete(
          "https://infinite-atoll-83522.herokuapp.com/api/shares/" +
